...
#
# Reads violation data from precheck result JSON, calls the configured LLM provider to
...
echo "Calling LLM provider for fix generation..."

SYSTEM_PROMPT_FILE="$(mktemp)"
USER_PROMPT_FILE="$(mktemp)"
LLM_OUTPUT_FILE="$(mktemp)"

cleanup_auto_fix_llm_files() {
  rm -f "$SYSTEM_PROMPT_FILE" "$USER_PROMPT_FILE" "$LLM_OUTPUT_FILE"
}

trap cleanup_auto_fix_llm_files EXIT
...
if LLM_SYSTEM_PROMPT_FILE="$SYSTEM_PROMPT_FILE" \
  ...
  bash "$SCRIPT_DIR/llm-message-client.sh"; then
  LLM_OUTPUT=$(cat "$LLM_OUTPUT_FILE")
fi

if [ -z "$LLM_OUTPUT" ]; then
  echo "LLM fix generation failed"
...
PATCH_FILE="$RESULTS_DIR/auto-fix-patch.sh"
{
  ...
  echo "# Generated by configured LLM provider -- review before applying"
  echo ""
  echo "cd \"${REPO_ROOT}\""
  echo ""
...
SUMMARY_FILE="$RESULTS_DIR/auto-fix-summary.md"
{
  echo "## Auto-Fix Report: $CHECK_ID"
  echo ""
  echo "Generated **${CMD_COUNT}** fix commands via configured LLM provider."
  echo ""
