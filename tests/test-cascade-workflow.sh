...
assert_contains "$workflow" 'preflight:'
assert_contains "$workflow" 'CASCADE_RESULTS_TOKEN_PRESENT: ${{ secrets.CASCADE_RESULTS_TOKEN != '\'''\'' }}'
assert_contains "$workflow" 'CASCADE_TOKEN_PRESENT: ${{ secrets.CASCADE_TOKEN != '\'''\'' }}'
assert_contains "$workflow" 'CASCADE_RESULTS_TOKEN secret is required before fan-out'
assert_contains "$workflow" 'needs: preflight'
assert_contains "$workflow" "if: always() && needs.fan-out.result != 'skipped'"
assert_contains "$workflow" 'while true; do'
assert_contains "$workflow" 'GH_TOKEN: ${{ secrets.CASCADE_RESULTS_TOKEN }}'
